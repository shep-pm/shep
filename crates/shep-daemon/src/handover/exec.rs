//! Becoming the successor: choosing a binary, writing the blob, and the
//! `execve` that carries a flock into a new image.
//!
//! [`exec_target`] resolves what to exec and never the running image, since
//! `/proc/self/exe` after an upgrade names a deleted inode. [`hand_over`]
//! then writes the blob, clears `FD_CLOEXEC` on the descriptors it names and
//! only those, and replaces this process. A failed exec unwinds both: the
//! blob comes off disk and every descriptor is close-on-exec again.
...
mod tests {
    ...
    use crate::handover::{CarriedFds, CarriedSheep, VERSION};
    ...
    const SELFTEST_NAME: &str =
        "handover::exec::tests::an_exec_replaces_the_image_and_keeps_a_descriptor";
