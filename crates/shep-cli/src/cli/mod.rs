...
// Nothing in a release build reaches the follow floor through this path: the
// one caller outside `flock` is lib.rs's own test module, which builds a
// `FlockArgs` by hand rather than parsing one.
