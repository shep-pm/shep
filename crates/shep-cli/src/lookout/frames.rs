...
frames.ansi renders all forty-two scenes through the same coloured
palette the pinned `.snap` tests use; read it with `less -R`. frames.txt
renders the same forty-two scenes through the flattened NO_COLOR palette
...
The last six frames are the full-screen bleats pane, `b` from the
dashboard, then the secrets pane, `S`, then the four sheep-pane scenes,
`↵` on a sheep. The seven before them are the settings screen, `s`
from the dashboard. It owns
