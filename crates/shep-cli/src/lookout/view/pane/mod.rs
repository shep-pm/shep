...
use self::field_row::{field_line, push_wrapped_blurb, section_header};
use self::layout::{body_width, lands_fits_beside_panel, line_columns, panel_width};
use self::list::list_lines;
// Named by path only from `view::fixtures`, which is `#[cfg(test)]`.
#[cfg(test)]
pub(super) use self::panel::panel_for_field;
pub(super) use self::panel::panel_lines;
...
mod tests {
    ...
    use super::fixtures::{blurb_anchor, marked, pane_to, screen_at, text_of, web_pane};
    use super::*;
