...
use crate::config::{AppConfig, CronParseError, CronSchedule, KillSignal, LevelMatcher};
use crate::secrets;
use crate::values::UpDuration;

use super::{NormalizeError, tilde, validate};
...
/// - [`NormalizeError::InvalidProbe`]: a `readiness_probe`/`liveness_probe` target [`ProbeTarget::parse`](crate::config::ProbeTarget::parse) rejects.
...
pub(super) const MIN_LIVENESS_INTERVAL: UpDuration = UpDuration::from_millis(1_000);
...
pub(super) const MIN_READINESS_INTERVAL: UpDuration = UpDuration::from_millis(1);
...
pub fn normalize_with_home(
    ...
    for (key, value) in &app.env {
        validate::validate_template(&app.name, &format!("env.{key}"), value, shep_home)?;
    }
    for (index, value) in app.args.iter().enumerate() {
        validate::validate_template(&app.name, &format!("args[{index}]"), value, shep_home)?;
    }
    for (field, value) in [("out_file", &app.out_file), ("err_file", &app.err_file)] {
        if let Some(value) = value {
            validate::validate_template(&app.name, field, value, shep_home)?;
    ...
    // a path.
    tilde::expand_paths(&mut app, home)?;
    if app.instances == 0 {
    ...
    // happens.
    validate::validate_watch_globs(&app.name, "watch_options", &app.watch_options)?;
    validate::validate_watch_globs(&app.name, "ignore_watch", &app.ignore_watch)?;
