...
`docs/writing-plans/plans/2026-08-27-dog-prerequisites.md:2124`. Shipped: `crates/shep-client/src/client.rs` carries `Reconnected`, `Client::reconnect` and `Client::reconnect_within`. Identity is `HelloAck::pid`, the only per-process fact the handshake carries; a handover `execve`s in place, so the pid and the instance-id counter cross it together and one reading answers both questions. The dog name a client handshook under rides every reconnect, for the reason `ReconnectingClient` re-sends it.
