...
- **Every task that changes what the pane draws invokes the `tui-screen-capture` skill, before writing the change and again after.** That is tasks 6, 7, 8, 10 and 11. Reading a diff is what fails to catch a TUI defect, and this plan's own testing leans on snapshot tests and rendered-line slices, which the skill names as the thing that pins a band whose background never got painted and reports it green. Use `--attrs` whenever the question is whether a band, a selected row or a chip is really painted rather than merely spelling its own name, and `--keys` to open the pane and read what came back. **Split the keystrokes across separate `--keys` arguments**, one per logical step, and give the capture a settle interval. A single combined string such as `--keys 'ejjjjjjj '` loses the trailing keys: they arrive in one read cycle and the app drops them, so the screen comes back looking as though the keypress did nothing. That misread cost a false bug report once already.
